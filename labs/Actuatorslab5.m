...
mOffScale = 1.05; % Start with 1 so both motors have same duty cycle.
...
while (toc < 1) % adjust the time to test if robot goes in straight line
...
minReflectance = [74.2,51.2,38.4,26.7,26.7,38.4]; % Set me to min reflectance 
...
maxReflectance = [846.9,740.7,549.8,432.8,416.8,454.6]; % Set me to max reflectance 
                                             % values for each sensor for
                                             % future reference
...
whiteThresh = 74.2; % Max value detected for all white
...
while (toc < 2.7)  % Adjust me if you want to stop your line following 
    ...
    error = 0.5*calibratedVals(1) + 0.52*calibratedVals(2) + 1*calibratedVals(3) - ...
            1*calibratedVals(4) - 1*calibratedVals(5) - 0.5*0.741*calibratedVals(6); % Designing this error term can sometimes be just as 
